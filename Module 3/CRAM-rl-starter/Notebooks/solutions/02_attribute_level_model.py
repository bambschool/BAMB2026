...
def _(mo):
    mo.md(
        ...
        In **Part 1** you measured, from the raw gaze, that the monkey samples
        **one attribute at a time** (reward *or* probability per look), and that
        Chip rides the **probability** bar (~93% of within-option dwell). The
        ...
        This part is the **adaptation**: make `gf` real so a fixation measures one
...
def _():
    ...
    return (
        DEFAULT_COST, NO_MAX, NHPMetaMDP, NHPMetaMDPEnv, SessionData, State,
        TimeStep, get_posterior_itrue, greedy_episode, np, plt, train,
    )
...
def _(SESSION, mo, scenes):
    mo.md(
        f"Loaded **{len(scenes)}** scenes from `{SESSION}` "
...
@app.cell
def _(feature_filter, np, verdict):
    def _ex1_checks():
        return [
...
def _():
    def decode_target(target):
        ...
        return option_rank, attribute
    return (decode_target,)


@app.cell
def _(decode_target, verdict):
    def _ex2_checks():
        return [
...
@app.cell
def _(NHPMetaMDPEnv, State, TimeStep, decode_target, feature_filter, np):
...
def _(np):
    def prob_fraction(decoded):
        ...
        # SOLUTION
        attrs = [a for (_, a) in decoded if a is not None]
        if not attrs:
            return float("nan")
        return sum(1 for a in attrs if a == 1) / len(attrs)
    return (prob_fraction,)


@app.cell
def _(np, prob_fraction, verdict):
    def _ex3_checks():
        return [
...
@app.cell
def _(AttrEnv, NHPMetaMDPEnv, greedy_episode, mdp, np, p_attr, p_object,
      prob_fraction, scenes):
...
def _(attr_stats, mo):
    ...
    mo.md(
        ...
        *same* policy with the *same* objective (identify the best option). The
        ...
        2. **rides the probability bar** (fixation fraction {attr_stats['prob_frac']:.2f}),
           matching Chip's 0.93 — and it sampled probability *across options*, i.e. it
           learned **direct attribute comparison** (the Perkins–Rich headline). Nobody
        ...
        **Answer.** Trained the same way on a **Dale** session, the model *still* goes
        probability-heavy — because probability is objectively more diagnostic. But the
        real Dale is **balanced** (~0.43 on probability, Part 1). So this model
        captures the *task-optimal* strategy and Chip, but not Dale's individual style.
        Reproducing that needs the **per-attribute preference** knob we flagged in
        Part 1 — a fitted weight/cost that makes an attribute more or less attractive
        than pure diagnosticity implies. That is the first thing we fit in the next
        part, and the point where the *model meets the individual monkey.*
        """
    )
    return
