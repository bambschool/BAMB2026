...
def _(mo):
    mo.md(
        ...
        time**, and **riding the probability bar** — matching **Chip** (≈0.93 of
        within-option dwell on probability).

        But there was an honest catch. Probability is *objectively* the more
...
def _():
    ...
    return (
        DEFAULT_COST, MAX_STEPS, NHPMetaMDP, NHPMetaMDPEnvAttr, NO_MAX, ROOT,
        SessionData, State, TimeStep, TrialScene, featurize, forward,
        masked_fix_probs, np, pd, plt, softmax, train,
    )
...
def _():
    def subjective_value(features, w_p):
        ...
        # SOLUTION
        return features[:, 0] + w_p * features[:, 1]
    return (subjective_value,)


@app.cell
def _(np, subjective_value, verdict):
    def _ex1_checks():
...
def _(np):
    def weight_terms(terms, w):
        ...
        # SOLUTION
        return np.sum(np.asarray(w)[None, :] * terms, axis=1)
    return (weight_terms,)


@app.cell
def _(np, verdict, weight_terms):
    def _ex2_checks():
...
@app.cell
def _(NHPMetaMDPEnvAttr, State, TimeStep, TrialScene, np, subjective_value,
      weight_terms):
    ...
    def make_subj_env(w_p):
        ...
        return SubjEnv, w
    return make_subj_env, subjectivize, weighted_posterior
...
@app.cell
def _(NO_MAX, chip_scenes, dale_scenes, make_subj_env, model_frac_prob, mdp,
      np, subjectivize, train):
...
def _(np):
    def nearest_fit(ws, fracs, target):
        ...
        # SOLUTION
        i = int(np.argmin(np.abs(np.asarray(fracs, float) - target)))
        return ws[i], fracs[i]
    return (nearest_fit,)


@app.cell
def _(nearest_fit, verdict):
    def _ex3_checks():
        ws = [0.8, 1.0, 1.2]
        fracs = [0.2, 0.5, 0.9]
        return [
...
@app.cell
def _(OBS_CHIP, OBS_DALE, W_GRID, curve, nearest_fit):
...
@app.cell
def _(OBS_CHIP, OBS_DALE, fit_done, frac_chip, frac_dale, mo, wp_chip,
      wp_dale):
...
@app.cell
def _(fit_done, frac_chip, frac_dale, verdict, wp_chip, wp_dale):
    def _fit_checks():
        if not fit_done:
